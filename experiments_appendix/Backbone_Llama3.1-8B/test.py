...
from transformers import LlamaTokenizerFast, LlamaConfig, LlamaForCausalLM, DataCollatorForSeq2Seq
...
tokenizer = LlamaTokenizerFast.from_pretrained(pretrained_model_name_or_path=args.model_path)
if args.device_id==-1:
    model = CustomLlamaForCausalLM.from_pretrained(pretrained_model_name_or_path=os.path.join(args.model_path, args.model_name), attn_implementation="flash_attention_2", torch_dtype="auto", device_map="auto").to(torch.bfloat16)
else:
    model = CustomLlamaForCausalLM.from_pretrained(pretrained_model_name_or_path=os.path.join(args.model_path, args.model_name), attn_implementation="flash_attention_2", torch_dtype="auto").to(torch.bfloat16).cuda(args.device_id)
print(model)
